import Link from "next/link";

export default function Footer() {
  return (
    <footer className="bg-[#2f4e8a] text-white/90">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-10 mb-12">
          {/* About Column */}
          <div>
            <h3 className="text-base font-semibold mb-5 text-white">About Jefferson County</h3>
            <ul className="space-y-3 text-sm">
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  County History
                </Link>
              </li>
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  Elected Officials
                </Link>
              </li>
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  Employment
                </Link>
              </li>
            </ul>
          </div>

          {/* Services Column */}
          <div>
            <h3 className="text-base font-semibold mb-5 text-white">Services</h3>
            <ul className="space-y-3 text-sm">
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  Permits &amp; Licenses
                </Link>
              </li>
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  Property Records
                </Link>
              </li>
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  Tax Information
                </Link>
              </li>
            </ul>
          </div>

          {/* Resources Column */}
          <div>
            <h3 className="text-base font-semibold mb-5 text-white">Resources</h3>
            <ul className="space-y-3 text-sm">
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  Forms &amp; Documents
                </Link>
              </li>
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  GIS Maps
                </Link>
              </li>
              <li>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                  Accessibility
                </Link>
              </li>
            </ul>
          </div>

          {/* Contact Column */}
          <div>
            <h3 className="text-base font-semibold mb-5 text-white">Contact Us</h3>
            <address className="not-italic text-sm space-y-2 leading-relaxed">
              <p>Jefferson County Courthouse</p>
              <p>100 E. Washington Street</p>
              <p>Charles Town, WV 25414</p>
              <p className="pt-3">Phone: (304) 728-3000</p>
            </address>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t-2 border-white/10 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <p className="text-sm text-white/80 text-center md:text-left">
              &copy; 2026 Jefferson County, West Virginia. All rights reserved.
            </p>
            <div className="flex items-center space-x-6">
              <Link href="/demo" className="text-sm hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                Privacy Policy
              </Link>
              <Link href="/demo" className="text-sm hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded">
                Terms of Use
              </Link>
              {/* Social Icons */}
              <div className="flex items-center space-x-3 ml-4">
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded" aria-label="Facebook">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                </Link>
                <Link href="/demo" className="hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded" aria-label="Twitter">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                  </svg>
                </Link>
              </div>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}

